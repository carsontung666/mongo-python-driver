...
# find_one() calls whose kwargs fall inside this set need none of Cursor's
...
class Collection(common.BaseObject, Generic[_DocumentType]):
    ...
    def find_one(
        ...
        if (
            ...
            # so Cursor raises its usual TypeError.
            len(args) < 2
            ...
            and type(self).find is Collection.find
    ...
    def _find_one_single_batch(
        ...
        """Run find_one's query without building a Cursor.
        ...
        # As in Cursor._query_spec(): a filter whose first key is "query"
        ...
        if not session:
            ...
            # As Cursor.__init__ does for an explicitly passed session.
            session._attached_to_cursor = True
        try:
            # As Cursor._get_read_preference(): an implicit session does
