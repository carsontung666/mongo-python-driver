...
class AsyncTestCollection(AsyncIntegrationTest):
    ...
    async def test_find_one_falls_back_for_class_level_patch_of_find(self):
        # mock.patch.object(AsyncCollection, "find") replaces the class
        # attribute, so identity against AsyncCollection.find cannot see it.
        ...
        with mock.patch.object(AsyncCollection, "find", recording):
            self.assertIsNotNone(await coll.find_one({"_id": 1}))
        self.assertEqual(1, len(calls))
