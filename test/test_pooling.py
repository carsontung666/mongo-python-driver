...
from pymongo import MongoClient, message, monitoring, timeout
...
class TestPooling(_TestPoolingBase):
    ...
    def test_get_conn_rolls_back_on_cancel_without_the_fast_path(self):
        ...
        self.assertIsNone(cx_pool._checkout_idle(), "fast path should decline")
        reused_context = conn.cancel_context
    ...
    def test_checkout_idle_declines_and_leaves_the_pool_untouched(self):
        ...
        def assert_declines(reason):
            self.assertIsNone(cx_pool._checkout_idle(), reason)
        ...
        cx_pool._check_interval_seconds = 0
        assert_declines("check interval reached")
        ...
        try:
            assert_declines("idle past maxIdleTimeMS")
        finally:
        ...
        try:
            assert_declines("stale generation")
        finally:
        ...
        self.assertIsNone(cx_pool._checkout_idle(), "no idle connection")
        ...
        self.assertIsNone(cx_pool._checkout_idle(), "pool is not ready")
        self.assertEqual(reset_before, snapshot(), "not-ready decline disturbed the pool")

    def test_checkout_idle_declines_when_requests_at_max(self):
        ...
        with cx_pool.checkout():
            pass
        conn = cx_pool._get_conn(0.0)  # requests == 1, queue empty
        with cx_pool.size_cond:
            cx_pool.conns.appendleft(conn)  # the long checkin's first half
        try:
            self.assertIsNone(cx_pool._checkout_idle())
            self.assertEqual(1, len(cx_pool.conns))
            self.assertEqual(1, cx_pool.requests)
        finally:
            with cx_pool.size_cond:
                cx_pool.conns.remove(conn)
            cx_pool.checkin(conn)
        self.assertEqual(0, cx_pool.requests)

    def test_checkout_idle_failure_pairs_the_checkout_started_event(self):
        ...
        cx_pool = self.create_pool(max_pool_size=1, event_listeners=_EventListeners([listener]))
        with cx_pool.checkout():
            pass
    ...
    def test_checkin_closes_stale_connection_before_releasing_the_slot(self):
